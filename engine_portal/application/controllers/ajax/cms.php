...
class Cms extends Ajax {
    ...
    function unset_as_featured(){
		$this->load->model('wow/items');
        $id = $this->input->post('id');
		$callback = $this->input->post('callback');
		
		$data = array('nominee' => 0);
    ...
    function wow_get_jury(){
        //load model
        $this->load->model('wow/jury');
        $MM = $this->jury;
        ...
        if ($totalSize > 0)
        {
            ...
            $result = $MM->get_join_jury($offset, $limit);
            
            $datalist = array();
            $this->load->model('wow/wow_event_m');
            foreach($result as $item){
                $item->edit_url = btn_edit('cms/wow/jury_edit/'.$item->id, 'title="Edit"');
                $item->delete_url = btn_delete('cms/wow/jury_delete/'.$item->id, 'title="Delete"');
                $event = $this->wow_event_m->get_select_where('name', array('id'=>$item->wow_event_id), TRUE);
                if ($event)
    ...
    function wow_get_items(){
        ...
        if ($totalSize > 0)
        {
            ...
            //$result = $MM->get_offset('*', $where, $offset, $limit);
            //replace $offset with $start for paging
            $result = $MM->get_offset('*', $where, $start, $limit);
            ...
            foreach($result as $item){
                ...
                if ($item->nominee==1){
                    $item->is_featured = '<a id="a_'.$item->id.'" href="javascript:unSetItemAsFeatured('.$item->id.');"><i class="icon-thumbs-up" title="unnominated"></i></a>';
                }else{
                    $item->is_featured = '<a id="a_'.$item->id.'" href="javascript:setItemAsFeatured('.$item->id.');"><i class="icon-hand-right" title="nominated"></i></a>';
                }
                $datalist [] = $item;
            }
            ...
            //create js paging
            $jsClick = 'javascript:loadItems('.$event_id.',$);';
            $this->_result['pagination'] = smart_paging_js($totalPages, $page, $jsClick, 7, '$');
        }
    }
    ...
    function wow_get_like(){
        ...
        if ($totalSize > 0)
        {
            ...
            foreach($result as $item){
                $item->delete_url = btn_delete('cms/wow/like_delete/'.$item_id.'/'.$item->id, 'title="Delete"');
    ...
    function wow_get_row(){
        ...
        if ($row->item_url!=''){
            $video[] = array(
                'item_url' => config_item('userfiles').'wow/'.$row->item_url,
                'type'     => 'video/mp4'
            );
        }
        $this->_result['item'] = $video;
    ...
    //wow static_content
    function wow_get_static(){
        //load model
        $this->load->model('wow/content_static');
        $MM = $this->content_static;
        ...
        if ($totalSize > 0)
        {
            ...
            $this->load->model('user_m');
            foreach($result as $item){
                $item->edit_url = btn_edit('cms/wow/static_edit/'.$item->id, 'title="Edit"');
                $item->delete_url = btn_delete('cms/wow/static_delete/'.$item->id, 'title="Delete"');
    ...
    function wow_get_event_by_category(){
        $this->load->helper('cms');
        $category_id = $this->input->post('category_id');
        $page = $this->input->post('page'); if (!$page) $page = 1;        
        $offset = $page-1;
        ...
        $totalSize = $this->wow_event_m->get_count($where);
        $totalPages = ceil($totalSize/$limit);
        
        $result = $this->wow_event_m->get_offset($fields, $where, $start, $limit);
            $datalist = array();
            foreach($result as $item){
                if($fields=='*'){
                    $item->edit_url = btn_edit('cms/wow/event_edit/'.$category_id.'/'.$item->id, 'title="Edit"');
                    $item->delete_url = btn_delete('cms/wow/event_delete/'.$item->id, 'title="Delete"');
                }
                $datalist [] = $item;
            }
        ...
        //create js paging
        $jsClick = 'javascript:loadGenreByCategory('.$category_id.',$);';
        $this->_result['pagination'] = smart_paging_js($totalPages, $page, $jsClick, 7, '$');
    }

    function wow_get_items_by_event(){
        //load model
        $this->load->model('wow/items');
        $MM = $this->items;
        ...
        if ($totalSize > 0)
        {
            ...
            $this->load->model('user_m');
            foreach($result as $item){
                $item->detail_url = btn_edit('cms/wow/like_detail/'.$item->id, 'title="Detail"');
                $datalist [] = $item;
            }
    ...
    function wow_static_update_toc(){
        $event_id = $this->input->post('event_id');
        $item_id = $this->input->post('item_id');

        //load model content_static
        $this->load->model('wow/content_static');

        //check flag toc by event_id
        $where = array('event_id'=>$event_id, 'toc'=>1);
        $flag_toc = $this->content_static->get_count($where);

        if($flag_toc == 0) {
            //do nothing
        } else {
            //update the items where toc = 1
            $this->db->query("UPDATE uz_wow_static SET toc = 0 WHERE id != '$item_id' AND event_id = '$event_id';");
        }
        
        $this->db->query("UPDATE uz_wow_static SET toc = 1 WHERE id = '$item_id';");
    }

    function wow_static_move_up(){
        $event_id = $this->input->post('event_id');
        $item_id = $this->input->post('item_id');
        ...
        //check nilai display_order dari item_id
        $where = array('id'=>$item_id);
        $display_order = $this->content_static->get($item_id, TRUE)->display_order;

        //get id where display_order = display_order - 1
        $where = array('display_order'=>$display_order-1, 'event_id'=>$event_id);
        $item_id_up = $this->content_static->get_select_where('*', $where, TRUE)->id;

        //update item_id_up : display_order + 1
        $this->db->query("UPDATE uz_wow_static SET display_order = display_order + 1 WHERE id = '$item_id_up';");
        //update item_id : display_order - 1
        $this->db->query("UPDATE uz_wow_static SET display_order = display_order - 1 WHERE id = '$item_id';");
    }

    function wow_static_move_down(){
        $event_id = $this->input->post('event_id');
        $item_id = $this->input->post('item_id');
        ...
        //get id where display_order = display_order + 1
        $where = array('display_order'=>$display_order+1, 'event_id'=>$event_id);
        $item_id_down = $this->content_static->get_select_where('*', $where, TRUE)->id;

        //update item_id_down : display_order - 1
        $this->db->query("UPDATE uz_wow_static SET display_order = display_order - 1 WHERE id = '$item_id_down';");
        //update item_id : display_order + 1
        $this->db->query("UPDATE uz_wow_static SET display_order = display_order + 1 WHERE id = '$item_id';");
    }

    function wow_approve_item(){
        $event_id = $this->input->post('event_id');
        $item_id = $this->input->post('item_id');
        ...
        //update value approved from uz_wow_items by item_id
        $where = array('id'=>$item_id, 'event_id'=>$event_id);
        $this->db->query("UPDATE uz_wow_items SET approved = 1 WHERE id = '$item_id' AND event_id = '$event_id';");
    }

    function wow_not_approve_item(){
        $event_id = $this->input->post('event_id');
        $item_id = $this->input->post('item_id');
        ...
        //update value approved from uz_wow_items by item_id
        $where = array('id'=>$item_id, 'event_id'=>$event_id);
        $this->db->query("UPDATE uz_wow_items SET approved = 0 WHERE id = '$item_id' AND event_id = '$event_id';");
    }

    function wow_get_event_field(){
        //load model
        ...
        if ($totalSize > 0)
        {
            ...
            $this->load->model('user_m');
            foreach($result as $item){
                $item->edit_url = btn_edit('cms/wow/event_field_edit/'.$item->id_event_field, 'title="Edit"');
                $item->delete_url = btn_delete('cms/wow/event_field_delete/'.$item->id_event_field, 'title="Delete"');
            ...
            //create js paging
            $jsClick = 'javascript:loadField('.$event_id.',$);';
            $this->_result['pagination'] = smart_paging_js($totalPages, $page, $jsClick, 7, '$');
        }
    }

    function wow_get_banner(){
        $this->load->helper('cms');
        $this->load->model('wow/wow_event_m');
        $event_id = $this->input->post('event_id');
        $page = $this->input->post('page'); if (!$page) $page = 1;        
        ...
        $totalSize = $this->banner->get_count($where);
        $totalPages = ceil($totalSize/$limit);
        
        $result = $this->banner->get_offset($fields, $where, $start, $limit);
            $datalist = array();
            foreach($result as $item){
                if($fields=='*'){
                    $item->edit_url = btn_edit('cms/wow/banner_edit/'.$event_id.'/'.$item->id, 'title="Edit"');
                    $item->delete_url = btn_delete('cms/wow/banner_delete/'.$item->id, 'title="Delete"');
                }
                $item->event_name = $this->wow_event_m->get($event_id, TRUE)->name;
                $datalist [] = $item;
            }
        ...
        //create js paging
        $jsClick = 'javascript:loadBanner('.$event_id.',$);';
        $this->_result['pagination'] = smart_paging_js($totalPages, $page, $jsClick, 7, '$');
    }

    function wow_get_field_value(){
        $this->load->helper('cms');
        $event_id = $this->input->post('event_id');
        $page = $this->input->post('page'); if (!$page) $page = 1;        
        $offset = $page-1;
        ...
        //load items model
        $this->load->model('wow/items');
        //get all items by event_id
        $where = array('event_id'=>$event_id);
        $totalSize = $this->items->get_count($where);
        $totalPages = ceil($totalSize/$limit);

        $item_by_event = $this->items->get_offset($fields, $where, $start, $limit);
        
        //load model event_field
        $this->load->model('wow/event_field');
        //get all event list by event_id
        $where = array('event_id'=>$event_id);
        $event_field = $this->event_field->get_select_where('*', $where);

        //load model items_field
        $this->load->model('wow/items_field');
        $listdata = array();
        foreach($item_by_event as $a){
            $get['item_id'] = $a->id;
            $get['item_name'] = $a->item_name;

            $get['edit_url'] = btn_edit('cms/wow/item_field_value_edit/'.$event_id.'/'.$a->id, 'title="Edit"');
            $get['delete_url'] = btn_delete('cms/wow/item_field_value_delete/'.$event_id.'/'.$a->id, 'title="Delete"');

            $loop = 0;
            foreach($event_field as $b){
                $where = array('id_items'=>$a->id,'id_event_field'=>$b->id_event_field);
                $data = $this->items_field->get_select_where('*', $where, TRUE);
                if ($data){
                    $get['field_value'][$loop] = $data->fieldvalue;
                } else {
                    $get['field_value'][$loop] = '-';
                }
                $loop++;
            }
        ...
        $this->_result['offset'] = $offset;
        $this->_result['start'] = $start;
        $this->_result['items'] = $listdata;        
        $this->_result['size'] = count($listdata);
        //tambahan
        $this->_result['event_field'] = $event_field;
        
        //create js paging
        $jsClick = 'javascript:loadFieldValue('.$event_id.',$);';
        $this->_result['pagination'] = smart_paging_js($totalPages, $page, $jsClick, 7, '$');
    }

    function wow_get_items_judge(){
        //load model
        $this->load->model('wow/items');
        $MM = $this->items;
        ...
        if ($totalSize > 0)
        {
            ...
            foreach($result as $item){
                ...
                //get nilai dari uz_wow_judge
                $where = array('judge_id'=>$judge_id, 'item_id'=>$item->id);
                $judge = $this->judge_m->get_select_where('*', $where, TRUE);
                if($judge){
                    $item->nilai = $judge->score;
                } else {
                    $item->nilai = '';
                }
    ...
    function wow_get_data_item_judge(){
        $judge_id = $this->session->userdata('userid');
        $item_id = $this->input->post('item_id');

        $this->load->model('wow/items');
        $item = $this->items->get($item_id, TRUE);

        //load model wow/judge_m
        $this->load->model('wow/judge_m');
        //get score & comment by item_id & judge_id
        $where = array('judge_id'=>$judge_id, 'item_id'=>$item_id);
        $score = $this->judge_m->get_select_where('*', $where, TRUE);

        if(!$score){
            $score = $this->judge_m->get_new();
        }
        $this->_result['score'] = $score;

        $this->_result['size'] = count($item);
        $this->_result['item'] = $item;

    }
	
	function wow_get_genre(){
        //load model
        $this->load->model('wow/genre');
        $MM = $this->genre;
        ...
        if ($totalSize > 0)
        {
            ...
            //$this->load->model('user_m');
            foreach($result as $item){
                $item->edit_url = btn_edit('cms/wow/genre_edit/'.$item->id, 'title="Edit"');
                $item->delete_url = btn_delete('cms/wow/genre_delete/'.$item->id, 'title="Delete"');
            ...
            //create js paging
            $jsClick = 'javascript:loadGenre('.$category_id.',$);';
            $this->_result['pagination'] = smart_paging_js($totalPages, $page, $jsClick, 7, '$');
        }
    }

    function wow_update_nilai_judge(){
        //load model wow/judge_m
        $this->load->model('wow/judge_m');

        $id = $this->input->post('id');
        $data_post = $this->judge_m->array_from_post(array('item_id','score','comment')); 
        ...
        $status = 0;
        if($id){
            if ($this->judge_m->save($data_post, $id) == TRUE){
                $status = 1;
            }
        }
        else {
	...
	function wow_get_news_eventorg(){
        $channel = 'wow';
        //load model
        ...
        //$where = array('id_category'=>$category_id);
        $where = array('channel_name'=>$channel, 'item_id'=>$event, 'type'=>$type);
        $totalSize = $MM->get_count($where);
        $totalPages = ceil($totalSize/$limit);
        ...
        if ($totalSize > 0)
        {
            ...
            foreach($result as $item){
                //get event name
                $item->event_name = $this->wow_event_m->get($item->item_id, TRUE)->name;

                //get u_name for list
                $item->news_by_name = $this->user_m->get($item->news_by, TRUE)->u_name;

                $item->edit_url = btn_edit('cms/wow/news_edit/'.$item->id, 'title="Edit"');
                $item->delete_url = btn_delete('cms/wow/news_delete/'.$item->id, 'title="Delete"');
            ...
            //create js paging
            $jsClick = 'javascript:loadNews('.$event.','.$type.',$);';
            $this->_result['pagination'] = smart_paging_js($totalPages, $page, $jsClick, 7, '$');
        }
    }

    function wow_news_active(){
        $id = $this->input->post('id');

        //load model news
        $this->load->model('news_m');
        $data_news = $this->news_m->get($id, TRUE);

        //cek data_news -> is_active

        if($data_news->is_active == 1){
            $this->db->query("UPDATE uz_channel_news SET is_active = 0 WHERE id = '$id';");
        } else {
            $this->db->query("UPDATE uz_channel_news SET is_active = 1 WHERE id = '$id';");
        }
    }
...
/*
 * file location: /engine/application/controllers/ajax/cms.php
 */
?>
