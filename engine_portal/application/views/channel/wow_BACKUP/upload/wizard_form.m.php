<style type="text/css">
    ...
    input[type="submit"] {
        border: 0;
        background: url('<?php echo config_item('assets_wow');?>images/button_submit.png') no-repeat center center;
        text-indent: -9999em;
        width: 70px;
        height: 25px;
    }
    ...
    input[type="submit"]:hover {
        border: 0;
        background: url('<?php echo config_item('assets_wow');?>images/button_submit_hover.png') no-repeat center center;
        text-indent: -9999em;
        width: 70px;
        height: 25px;
    }
    input[type="reset"] {
        border: 0;
        background: url('<?php echo config_item('assets_wow');?>images/button_reset.png') no-repeat center center;
        text-indent: -9999em;
        width: 70px;
        height: 25px;
    }
    input[type="reset"]:hover {
        border: 0;
        background: url('<?php echo config_item('assets_wow');?>images/button_reset_hover.png') no-repeat center center;
        text-indent: -9999em;
        width: 70px;
        height: 25px;
    }
...
<div class="container kecil" style="margin-top:10px;">
    <form id="frm_wow" name="frm_wow" method="post" enctype="multipart/form-data">
        <table class="tabel-upload">
            ...
            <tr>
                <td colspan="2">
                    <input class="upload" type="submit" value="SUBMIT DATA" style="display:none;" />
                    <input class="upload" type="reset" value="RESET" style="display:none;" />
                    <input class="back" type="button" value="CANCEL" onclick="startWizard(false);" />
                    
                    <input class="cancel-upload" type="button" value="CANCEL UPLOAD" onclick="cancelUpload();" style="display:none;" />
                </td>
            </tr>
        </table>
    </form>
        ...
        $('input[type="file"]').change(function(){
            ...
            if (theName==='video'){
                ...
                if (typeof file_size!=='undefined' && file_size > allowed_movie_size){
                    alert('Ukuran file anda melebih dari yang diijinkan. Pastikan ukuran file upload anda maks ' + parseInt(allowed_movie_type/1024/1024)+'MB');
                    $(this).val('');
                    return;
                }
            }else{
