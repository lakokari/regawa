...
$.fn.swipePlanes = function(opciones) {
    return this.each(function() {
        ...
        metaPaginas[metaPagNum] -= anchoVisor - (bufferPaginas - metaPaginas[metaPagNum]);
        divContenedor.css('width', bufferPaginas + 'px');
        generarControles($(this), metaPagNum);
        var iX = 0, aX = 0, fX = 0;
        var drag = 0, t = 0, ms = 0, metaEvento, dX = 0;
        $(this).bind('touchstart', function(event) {
            clearInterval(t);
            if (event.pageX)
                iX = event.pageX;
            if (event.originalEvent.touches)
                iX = event.originalEvent.touches[0].pageX;
            divContenedor.css('transition', 'none');
            drag = 1;
            ms = 0;
            metaEvento = 'click';
        ...
        $(this).bind('touchmove', function(event) {
            if (event.pageX)
                aX = event.pageX;
            if (event.originalEvent.touches)
                aX = event.originalEvent.touches[0].pageX;
            dX = Math.abs(aX - iX);
            if (dX > 50)
        ...
        $(this).bind('touchend touchcancel touchleave', function(event) {
            if (drag && dX > 20) {
                ...
                if (event.originalEvent.touches)
                    fX = aX;
                if ((iX - fX) > 0 && ms < 100 && dX > 50)
                    metaEvento = 'swipeLeft';
                if ((iX - fX) <= 0 && ms < 100 && dX > 50)
                    metaEvento = 'swipeRight';
                if ((iX - fX) > 0 && ms > 100 && dX > 50)
                    metaEvento = 'moveLeft';
                if ((iX - fX) <= 0 && ms > 100 && dX > 50)
                    metaEvento = 'moveRight';
                if (dX <= 50)
                    metaEvento = 'click';
                switch (metaEvento) {
                    case'swipeLeft':
                        if (pagActual < metaPagNum) {
                            pagActual++
                        }
                        break;
                    case'moveLeft':
                        if (pagActual < metaPagNum && dX > (Math.abs(anchoVisor / 2))) {
                            pagActual++
                        }
                        break;
                    case'swipeRight':
                        ...
                        break;
                    case'moveRight':
                        if (pagActual > 0 && dX > (Math.abs(anchoVisor / 2))) {
                            pagActual--
                        }
                        break
                }
        ...
        function generarControles(divVisor, metaPagNum) {
            ...
            $('.' + randId + 'swipePlanesRowLeft').css('border-top-right-radius', '100px');
            $('.' + randId + 'swipePlanesRowLeft').css('border-bottom-right-radius', '100px');
            ...
            $('.' + randId + 'swipePlanesRowRight').css('border-top-left-radius', '100px');
            $('.' + randId + 'swipePlanesRowRight').css('border-bottom-left-radius', '100px');
            ...
            $('.' + randId + 'swipePlanesPaginator').css('display', 'none');
